...
class Captcha
{
	...
	public function __construct()
	{
		...
		$this->cache = new \think\cache\driver\Redis(config('cache.stores.redis'));
		$this->cache_prefix = 'captcha_' . config('app.app_name');
	}
	...
	public function generate(): array
	{
		...
		if ($type == 1) {
			...
			$code = mb_strtolower($bag, 'UTF-8');
			$hash = password_hash($code, PASSWORD_BCRYPT, ['cost' => 10]);
		}
		
		$key = md5(uniqid() . mt_rand(100000, 999999));
		$this->cache->set("{$this->cache_prefix}_{$key}", $hash, $this->expire);
	...
	{
		$cache_name = "{$this->cache_prefix}_{$key}";
		
		$hash = $this->cache->get($cache_name);
		if (!$hash) {
			return false;
		}
		...
		// $hash2 = password_hash($code, PASSWORD_BCRYPT, ['cost' => 10]);
	...
	public function create(string $config = null)
	{
		...
		// 绘验证码
		$text = $this->useZh ? preg_split('/(?<!^)(?!$)/u', $bag) : str_split($bag); // 验证码

...
class MoneyLogController extends AuthController
{
	...
	protected function _search($params = [], $is_export = 0)
	{
		...
		foreach ($list as $model)
		{
			$tmp = [];
			$tmp['id'] = $model->id;
			$tmp['out_trade_no'] = $model->type == 2 && $model->order ? $model->order->out_trade_no : '';
		...
		if (!$is_export)
		{
			...
			// 总费用
			$data['info']['total_fee'] = BusinessMoneyLog::where($_where)->where('type', '=', 2)->sum('money');
			// $data['info']['total_fee'] = abs($data['info']['total_fee']);

			// 交易笔数
			$data['info']['total_order'] = BusinessMoneyLog::where($_where)->where('type', '=', 2)->count('id');

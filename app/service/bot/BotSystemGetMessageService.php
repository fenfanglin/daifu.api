...
use app\model\Order;
use app\service\bot\BotDingxintongService;
...
⚠️如遇到，当笔充值与上笔充值地址有变的，需我方至少两人同时确认",
		...
		//回单
		if (strpos($text, 'hd-') !== false)
		{
			$dd = str_replace(substr($text, 0, 3), '', $text);
			$order_info = $this->bot_function->getOrder($dd);
			if (!$order_info)
			{
				$send_message = "订单不存在！";
			...
			else
			{
				...
				if (isset($info['order_id']) && $info['order_id'])
				{
					...
					$bot_dxt = new BotDingxintongService($this->token, $this->message_data['message']['message_id'], $order_info);
					$result = $bot_dxt->get_pdf($out_trade_no, $chatId, $text);
					exit;
				}
				else
				{
...
//                break;
	...
	public function checkOperator($chatId, $username)
	{
		...
		$admin = ['JQK999999999', 'JQK6666666', 'JQK777777', 'JQK55555', 'JQK188888', 'JQK181818', 'hhllip', 'HongLeongPay', 'HongLeongPay99', 'pao668868', 'hongleongpay77', 'hongleongpay66'];

...
class DemoParam extends BaseModel
{
	protected $table = 'pay_demo_param';
	...
	{
		$info = self::where('key', $key)->value('info');
		$info = json_decode($info, true);

		return $info ?? [];
	}

	public static function saveInfo($key, $info)
	{
		$info = json_encode($info, JSON_UNESCAPED_UNICODE);
		self::where('key', $key)->save(['info' => $info]);
	}
}

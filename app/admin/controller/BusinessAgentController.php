...
class BusinessAgentController extends AuthController
{
	private $controller_name = '四方';
	...
	protected function _search($params = [], $is_export = 0)
	{
		...
		$query->where('type', 1);         //类型：1代理 2工作室 3商户
		...
		foreach ($list as $model)
		{
			...
			$tmp['order_rate'] = $model->order_rate;
			if ($model->role)
			{
	...
	public function view()
	{
		...
		$data['system_logo'] = $model->system_logo;
	...
	public function save()
	{
		...
		$rule = [
			...
			'order_rate|订单费率' => 'require|float|between:0.0001,0.0999',
		];
		...
		else
		{
			...
			$model->type = 1; //类型：1代理 2工作室 3商户
		}

		$model->realname = input('post.realname');
		$model->remark = input('post.remark');
		$model->system_name = input('post.system_name');
		$model->system_logo = input('post.system_logo');
	...
	public function save_recharge()
	{
		...
		{
			$remark = "【{$user->id}】" . input('post.remark');
		}
